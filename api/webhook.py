...
DEFAULT_TZ = os.getenv("APP_TZ", "Europe/Berlin")
...
def _format_list_with_ru(items: List[str]) -> str:
    lines = []
    for it in items:
        ru_known: Optional[Tuple[str, bool]] = ru_name_for(it)
        if ru_known and ru_known[0] and ru_known[1]:
            lines.append(f"• {ru_known[0]}")
        else:
            lines.append(f"• {it}")
...
async def _send_start(chat_id: int):
    ...
    # группируем по турнирам, отбрасывая категории 15/25/50
...
async def _handle_watch(chat_id: int, payload: str):
    ...
    for nm in names:
        ...
        pair = ru_name_for(en_full)  # Optional[(ru, known)]
        if pair is None or not pair[1] or not pair[0]:
            # RU неизвестно — спросим и запомним pending
            ...
            # RU есть — добавляем в наблюдаемые
