...
DDL = [
    "CREATE TABLE IF NOT EXISTS users (chat_id BIGINT PRIMARY KEY, tz TEXT DEFAULT 'Europe/Helsinki', created_at TIMESTAMPTZ DEFAULT NOW());",
    "CREATE TABLE IF NOT EXISTS watchlist (id BIGSERIAL PRIMARY KEY, chat_id BIGINT NOT NULL, label TEXT NOT NULL, resolved_name TEXT, provider TEXT NOT NULL, provider_player_id TEXT, expires_on DATE, created_at TIMESTAMPTZ DEFAULT NOW());",
    "CREATE UNIQUE INDEX IF NOT EXISTS u_watchlist_daily ON watchlist(chat_id, label, provider, expires_on);",
    "CREATE TABLE IF NOT EXISTS notified (id BIGSERIAL PRIMARY KEY, chat_id BIGINT NOT NULL, provider TEXT NOT NULL, event_id TEXT NOT NULL, event_day DATE NOT NULL, sent_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(chat_id, provider, event_id, event_day));"
]
...
def add_watch(chat_id: int, label: str, provider: str, expires_on: date, resolved_name: str = None, provider_player_id: str = None):
    with _conn() as con, con.cursor() as cur:
        cur.execute("INSERT INTO watchlist(chat_id,label,provider,expires_on,resolved_name,provider_player_id) VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING", (chat_id, label, provider, expires_on, resolved_name, provider_player_id))
...
def mark_notified(chat_id: int, provider: str, event_id: str, event_day: date):
    with _conn() as con, con.cursor() as cur:
        cur.execute("INSERT INTO notified(chat_id, provider, event_id, event_day) VALUES (%s,%s,%s,%s) ON CONFLICT DO NOTHING", (chat_id, provider, event_id, event_day))
