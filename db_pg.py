...
def ensure_schema() -> None:
    with _conn() as con, con.cursor() as cur:
        ...
        """)
        # events_cache (и авто-миграции столбцов)
        cur.execute("create table if not exists events_cache (ds date primary key)")
        cur.execute("alter table events_cache add column if not exists data jsonb")
        cur.execute("alter table events_cache alter column data set default '{}'::jsonb")
        cur.execute("alter table events_cache add column if not exists updated_at timestamptz not null default now()")
...
def ru_name_for(en_full: str) -> Tuple[str, bool]:
    """Всегда возвращает (ru, known). known=True если alias есть, иначе False."""
    en_full = (en_full or "").strip()
    if not en_full:
        return ("", False)
    with _conn() as con, con.cursor() as cur:
...
def list_today(chat_id: int, day: dt.date) -> List[str]:
    with _conn() as con, con.cursor() as cur:
        cur.execute("select name_en from watches where chat_id=%s and day=%s order by name_en",
                    (chat_id, day))
