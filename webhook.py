...
async def webhook(req: Request):
    # Validate secret header from Telegram (set via setWebhook secret_token)
    if WEBHOOK_SECRET:
    ...
    if text.startswith("/start") or text.startswith("/help"):
        ...
        help_text = (
            ...
            "/list — показ списка на сегодня\n"
            "/clear — очистить список\n"
            "/tz Europe/Helsinki — поменять часовой пояс\n"
            "/format — пример сообщения\n\n"
            "Я работаю на Vercel: оповещения приходят по крону каждые несколько минут."
        )
